...
class MaskedBackbone(nn.Module):
    """Thin wrapper around D2's backbone to provide padding masking."""
    ...
    def __init__(self, cfg):
        ...
        self.feature_strides = [
            backbone_shape[f].stride for f in backbone_shape
        ]
        self.num_channels = backbone_shape[
            list(backbone_shape.keys())[-1]
        ].channels
...
class Detr(nn.Module):
    ...
    def inference(self, box_cls, box_pred, mask_pred, image_sizes):
        """Compute the model's outputs in inference mode.
